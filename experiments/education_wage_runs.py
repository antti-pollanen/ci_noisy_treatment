...
import data.education_wage_augmented_data_generator as education_wage_data_generator
...
def get_average_interventional_distance_education_wage(
    z_test: np.ndarray,
    t_test: np.ndarray,
    y_min: float,
    y_max: float,
    ...
    y_mean_func_gt,
    y_sd_gt: float,
    z_train_and_validation: np.ndarray,
):
    ...
    assert len(t_test.shape) == 1 or (
        len(t_test.shape) == 2 and t_test.shape[1] == 1
    ), t_test.shape
    assert len(z_test.shape) == 2, z_test.shape
    assert z_test.shape[0] == t_test.shape[0], (z_test.shape, t_test.shape)

    t_test.shape = (-1, 1)

    y_uniform_sample = np.random.uniform(y_min, y_max, size=t_test.shape)

    print("Computing p(y|do(t)) estimates")
    densities_estimate = run_common.get_y_do_t_density(
        y=y_uniform_sample,
        t=t_test,
        y_mean_func=y_mean_func_estimate,
        y_sd=y_sd_estimate,
        z_for_expectation=z_train_and_validation,
    )

    print("Computing p(y|do(t)) ground truths")
    densities_gt = run_common.get_y_do_t_density(
        y=y_uniform_sample,
        t=t_test,
        y_mean_func=y_mean_func_gt,
        y_sd=y_sd_gt,
        z_for_expectation=z_test,
    )

    assert densities_estimate.shape == densities_gt.shape == (t_test.shape[0],), (
...
def education_wage_ceme_run(params):

    if params["uses_known_w_sd"] and params["relative_w_sd"] == 0.0:
        output = {}
        output["params"] = params
        print("Skipping run with known w_sd and relative_w_sd == 0.0")
        print("Output:")
    ...
    data_generator = education_wage_data_generator.Generator(
        relative_w_sd=params["relative_w_sd"]
    )
    ...
    output["y_idrf_uniform_error"] = run_common.y_idrf_uniform(
        data_generator.z,
        data_generator.t,
        model,
        "CEME",
        data_generator.y_mean_func,
        n_samples=10000,
    )
    ...
    (
    ...
    ) = get_average_interventional_distance_education_wage(
        z_test=test_data.z,
        t_test=test_data.t,
        y_min=y_min,
        y_max=y_max,
        ...
        z_train_and_validation=np.concatenate((train_data.z, validate_data.z), axis=0),
    )
...
def education_wage_mlp_run(params):
    print(f"Running synthetic_ceme_run with params: {params}")
